...
I wanted to keep a small form factor, so that lead to choosing an ITX case. The NAS-N150-8P has been popular, so I opted for the N150 version since it was the only thing in stock. I don't plan on heavy VM usage. If I am going to do VMs, I could build another machine for that. 

I wanted to max out the RAM, but I waited too long and RAM prices shot up and went out of stocks. Eventually got my hands on 8 gigabytes of Crucial RAM and SSD. I choose Crucial, because I was worried about compatibility issues.
...
2. Remove 2 thumbsscrews to remove bottom fan panel.
...
6. Push in the power connect into the PSU.
...
1. Push in the back panel plate until it snaps into black. The protrusions should be outside the case.
...
5. Plug in SFF-8643 into motheroard and feedthrough hole to reach backplane.
6. Plug in SATA1 - SATA8 corresponding to the motherboard. SATA1 is tothe left of the case if you are looking at the case.
...
7. Web UI Authetnication. Chose Configure using Web UI over Administrative user.
8. Wait for system to boot up.
9. Take note of ip addresss.
10. Use console option 4) Set up local adminstrator to setup password for truenas_admin.
